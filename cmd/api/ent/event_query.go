...
func (eq *EventQuery) QueryOwner() *SignalQuery {
	query := &SignalQuery{config: eq.config}
...
func (eq *EventQuery) WithOwner(opts ...func(*SignalQuery)) *EventQuery {
	query := &SignalQuery{config: eq.config}
...
func (eq *EventQuery) sqlAll(ctx context.Context) ([]*Event, error) {
	...
	if query := eq.withOwner; query != nil {
		...
		for i := range nodes {
			if fk := nodes[i].signal_events; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "signal_events" returned %v`, n.ID)
			}
			for i := range nodes {
